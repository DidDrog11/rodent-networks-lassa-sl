...
Lassa fever, from *Mammarenavirus lassaense* (LASV), is an endemic zoonosis in West Africa. Human infections arise from rodent-to-human transmission, mainly from the synanthropic reservoir *Mastomys natalensis*. In Sierra Leone, small-mammal communities vary across land use gradients, shaping LASV transmission risk. How anthropogenic environments facilitate these interactions remains poorly understood.
...
Agricultural communities supported more distinct species (higher richness) and had denser networks (higher degree) than villages and forests, though space-sharing rates did not differ substantially. Network topology varied by land use; village networks were more modular (i.e., fragmented into distinct interaction clusters). Notably, intraspecific space sharing among *M. natalensis* was more frequent in agriculture than villages, suggesting land use modulates intraspecific transmission.

LASV seroprevalence was 5.7% across the community, with antibodies in nine species. We found no significant association between seroprevalence and land use or overall network structure. These findings show simple ecological drivers do not fully explain LASV exposure patterns, highlighting that species-specific space use and habitat-dependent interactions are crucial for identifying transmission pathways and high-risk settings.
...
We analyzed the structure of the space-sharing networks at three distinct levels: 1) the overall network within each land use type, 2) the individual animals (nodes) within those networks, and 3) the specific roles of and interactions between species. 

First, to characterize the overall networks, we calculated global metrics for each land use type (village, agriculture, and forest). These included the total number of nodes (animals) and edges (inferred contacts), as well as the network-wide mean degree and betweenness centrality. 

Second, to evaluate the roles of individual animals, we calculated two key node-level centrality measures. We used degree centrality, the number of connections for each animal, as a direct measure of its contact frequency. We used betweenness centrality to identify individuals acting as critical bridges within the network by measuring how often they fell on the shortest path between other nodes. Finally, we investigated species-specific interaction patterns. We analyzed the degree distribution for each species to identify which were the most connected in each habitat. 
...
The number of individuals (nodes) was highest in agricultural settings (n = 379) compared to villages (n = 261) and forests (n = 44). Networks in village settings exhibited the highest overall connectivity, as indicated by the highest mean degree (mean = 3.4, standard deviation (SD) = 3.6), followed by agricultural (mean = 1.9, SD = 2.3) and forest settings (mean = 1.4, SD = 1.2). Additionally, maximum degree values—indicating individuals with the greatest number of connections—were observed in village (degree = 14) and agricultural (degree = 13) settings.

Mean betweenness centrality was highest in agricultural (mean = 1.4, SD = 8.1) and forest (mean = 1.4, SD = 4.7) settings, and lowest in villages (mean = 0.9, SD = 3.3). Network modularity, a measure of community fragmentation, was highest in agricultural settings (mean = 0.75, SD = 0.08), indicating a greater tendency for networks to separate into distinct clusters. Modularity was lower in villages (mean = 0.51, SD = 0.23) and lowest in forests (mean = 0.27, SD = 0.26).

Degree centrality varied across species, with the highest values typically observed in species more common in agricultural settings. Native rodent species *L. sikapusi*, *M. setulosus*, *P. rostratus*, and the shrew *C. olivieri* had individuals with degrees up to 13, though most were lower values (Figure 3). In villages, the invasive synanthropes *M. musculus* and *R. rattus* reached maximum degrees of 14 and 11 with high mean degrees (4.1 and 4.7 respectively). By contrast, M. natalensis, though abundant in both agricultural and village settings, had a lower maximum degree (8 in villages, 6 in agriculture) and similar mean values (2.3 and 1.8, respectively).
...
##  Network Position and LASV Seropositivity

We found a significant association between an individual's number of contacts and its serostatus. Rodents and shrews that were LASV seropositive had a significantly lower mean degree centrality (mean = 3.7, SD = 2.9) compared to seronegative individuals (mean = 5.5, SD = 5.1) (W = 10018, p = 0.039). This pattern held for *M. natalensis* (W = 354, *p* = 0.048) and *L. sikapusi* (W = 99.5, *p* = 0.03), but not for *C. olivieri* (W = 429.5, *p* = 0.54).

In contrast, we found no significant difference in betweenness centrality between seropositive and seronegative individuals. This was true both for the overall community and within species-specific analyses (e.g., *M. natalensis*, W = 585.5, *p* = 0.63). This suggests that while infected animals tended to have fewer direct contacts, their role as bridges connecting different parts of the network was not distinct from that of uninfected animals.

## Association between Contact Patterns and LASV Seropositivity

In our model for *M. natalensis*, an individual's odds of being LASV seropositive increased with its total number of contacts (degree; OR = 1.25, 95% CrI: 1.10–1.40). We also found some evidence that risk increased with a higher proportion of conspecific contacts (homophily; OR = 2.02, 95% CrI: 1.00–3.96).

Critically, we identified a strong negative interaction between degree and homophily (OR = 0.52, 95% CrI: 0.40–0.67). This indicates that while having more contacts increased risk overall, this effect was significantly weaker for individuals whose contacts were primarily with other *M. natalensis*. In other words, for highly connected animals, contacts with other species were more strongly associated with seropositivity than contacts with their own species.

# Discussion

In the Eastern Province of Sierra Leone, we found that small-mammal contact networks varied significantly by habitat. Networks were most densely connected in village settings (mean degree = 3.4), followed by agricultural (1.9) and forest (1.4) habitats. While *M. natalensis* exhibited a high probability of intra-specific contacts in agricultural settings—suggesting these areas may serve as focal points for LASV transmission, consistent with findings linking anthropogenic environments to zoonotic disease risk [@pei_anthropogenic_2024; @carlson_pathogens_2025]. LASV seropositivity was detected at low prevalence across nine rodent and shrew species, with *M. natalensis* comprising most seropositive individuals. However, a key finding was that seropositive individuals had lower degree centrality overall, though this pattern was not consistent across all species
...
Although we hypothesised that contact rates would be higher in anthropogenic habitats, global degree centrality was similar across land use types. However, the upper tail of the degree distribution was skewed towards individuals in villages and agriculture, underscoring the importance of individual-level variation not captured by aggregated metrics [@farine_constructing_2015; @silk_understanding_2017]. Agricultural areas also exhibited the highest rarefied species richness and a greater proportion of inter-specific space-sharing, likely reflecting edge effects that facilitate interactions between synanthropic and sylvatic species [@despommier_role_2007; @pruvot_small_2024]. Notably, native species such as *P. rostratus* in agricultural habitats displayed high degree centrality, suggesting that suggesting that a few highly interactive species may disproportionately shape network connectivity and transmission risk [@gibb_zoonotic_2020].

Network structure also varied across land use types. Agricultural and forest networks had higher mean betweenness centrality than village networks, indicating that individuals in those habitats more frequently acted as bridges connecting otherwise separate parts of the network. This suggests more cohesive contact structures in agricultural and forest settings, potentially supporting faster transmission cycles [@keeling_networks_2005].
...
The structure of *M. natalensis* interactions also varied by land use. In agricultural settings, *M. natalensis* exhibited significantly higher odds of intra-specific clustering consistent with the species' weak territoriality [@anderson_density_1961; @whisson_home_2007; @borremans_happily_2014; @hoffmann_lassa_2024]. In villages, this strong tendency for intra-specific contact was not statistically significant, and the odds of forming ties with other species were reduced. This suggests that while *M. natalensis* may have a more diverse range of inter-specific encounters in villages proportionally, their overall social interactions are less frequent compared to other species in that habitat. This dynamic could dilute intra-specific transmission chains while potentially creating varied, if infrequent, spillover pathways. Movement between habitats by individuals driven by resource availability may further further modulate these dynamics [@marien_movement_2018].

The significant negative interaction between degree and homophily reveals a potential ecological dynamic: for individuals with a limited number of social partners, transmission risk appears tightly linked to contact with conspecifics (*M. natalensis*). However, for highly-connected 'hub' individuals, this specificity may break down. Their increased risk appears to be driven by the sheer volume of contacts, irrespective of species, suggesting that inter-species transmission events may be particularly important at these high-traffic nodes in the network. This finding could challenge the paradigm of *M. natalensis* as the sole driver of transmission, pointing towards a more complex, community-level maintenance system, especially in settings that foster high connectivity. However, as this analysis was exploratory and based on simulated networks, these findings must be interpreted with caution pending further validation.
...
The seroprevalence of LASV (5.7%), was consistent with prior estimates from Sierra Leone [@bangura_lassa_2021]. Our study included forest sites further from human habitation, yet the proportion of *M. natalensis* individuals testing positive was similar (~9%). However, *M. natalensis* represented a smaller fraction of total seropositive individuals (28% vs. 75%), and we detected LASV antibodies in additional species (*C. olivieri*, *P. rostratus*, *M. setulosus*, *Malacomys edwardsi*, *Hybomys planifrons* and *Mastomys erythroleucus*), suggesting broader host involvement than previously documented [@bangura_lassa_2021]. This finding supports a multi-host perspective on LASV ecology [@demby_lassa_2001; @olayemi_new_2016].

Several key assumptions of our network inference methods warrant consideration. First, contacts were inferred from spatio-temporal co-occurrence, which assumes that proximity is a reliable proxy for interactions capable of transmission. This assumption may not hold uniformly across species with variable movement behaviours and social structures [@perkins_comparison_2009]. Although our use of species-specific home range radii provides a more ecologically realistic model, our approach must still assume that a capture location represents a central point within an individual's short-term activity space [@wanelik_new_2022]. While real-time tracking (e.g., radio-tagging) could refine these estimates, significant ethical and logistical challenges currently preclude their use in our study communities [@mohr_monitoring_2007; @borremans_nonlinear_2017].

Second, our trapping methodology has inherent limitations. Like all trapping studies, we captured only a subset of the active individuals, and unobserved behavioural differences between captured and uncaptured animals could bias network inferences [@parmenter_small-mammal_2003; @moore_modeling_2005]. Furthermore, removal trapping may have created temporary spatial vacuums filled by transient individuals with potentially different contact behaviours [@sullivanInfluenceRemovalSampling2013]. Although trapping sessions were spaced by ≥3 months to allow for community re-equilibration, as recommended in regional protocols, we cannot exclude residual bias from altered community composition. Our leave-one-out meta-analysis helps account for spatial hotspots between trapping sessions, but it cannot fully address structural changes caused by the removal and replacement of key individuals.

Ultimately, replicating the study across a greater number of sites would be valuable for evaluating the consistency of these patterns and the potential impact of these methodological biases.
